...
def load_pit_stop_data():
    """Load pit stop data"""
    try:
        df = pd.read_csv("tables/pit_stop_analysis_by_strategy_2022_2024.csv")
        return df
    except FileNotFoundError:
        print(
            "Pit stop data file not found: tables/pit_stop_analysis_by_strategy_2022_2024.csv"
        )
        return None
...
def load_pitlane_time_data():
    ...
    try:
        df = pd.read_csv("tables/pitlane_time.csv")
        return df
    except FileNotFoundError:
        print("Pitlane time data file not found: tables/pitlane_time.csv")
        return None
...
def simulate_race_with_pit_stops(
    ...
    for lap in range(1, num_laps + 1):
        ...
        # Base lap time adjusted by RO value (higher RO = faster base lap time)
        # Assuming base lap time of 95 seconds, RO range 288-310, 0.05s reduction per RO unit
        ...
        noise_std = calculate_dr_based_std(
            driver_info["DR_Value"], dr_min, dr_max, 0.25
        )
